...
int main(int argc, char* argv[])
{
    ...
    if (!outfile) {
        ...
        vector <double> alpha_values = {0., 1., 5., 10.};
        vector <double> sigma_values = {0.5};
    ...
    for (double alpha : alpha_values)
    {
        for (double sigma : sigma_values)
        {
            ...
            for (int i = 0; i < samples; ++i)
                integrate_nonlineardynamics(alpha, b0, d, sigma, tau, 1.0, 0.5, dt, tmax, 10.0, traj_x, traj_env);

                double xmean, xstd;
                calculate_mean_and_std(traj_x, xmean, xstd);

                outfile << alpha << " " << sigma << " " << xmean << " " << xstd << " " << endl;
                for (const auto& value : traj_x) {
                    outfile << value << " ";
                }
                outfile << endl << endl;
